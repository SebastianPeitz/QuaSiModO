...
#     # R = np.array([[1e-4]])
#     # S = np.array([[0.0]])
#     return np.trapz(np.sum(np.power(alpha, 2), axis=1), dx=h) #+ \
#            # np.trapz(np.diag(u @ R @ u.T), dx=h) + \
#            # np.trapz(np.diag(deltaU @ S @ deltaU.T), dx=h)
#
#
...
def createSurrogateModel(modelData, data):
    ...
    for i in range(nSys):
        ...
        U = Y[i].transpose() @ V[:, I[:modelData.nModes + 1]]
